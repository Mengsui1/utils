...
class CutImage{
    ...
    //检查参数
    check(obj){
        if(!(obj.bgCanvas instanceof HTMLElement && obj.cutCanvas instanceof HTMLElement)){
    ...
    _drawImage(){
        ...
        this.cutCtx.drawImage(
            options.img,
            cut.sx,cut.sy,
            cut.sWidth,cut.sHeight,
            0,0,
            this.cutPos.width,this.cutPos.height
        )
    }
    //画布移动边界值判断
    isrect(x, y) {
        ...
        const minX =
            -this.drawOption.width * this.nowScale + this.cutPos.right;
        const maxY = Math.abs(this.bgPos.top - this.cutPos.top);
        const minY =
            -this.drawOption.height * this.nowScale +
            this.cutPos.bottom;
    ...
    touchMove(e){
        ...
        if (touches.length === 1) {
            ...
            this.isrect(
                this.drawOption.x - difX,
                this.drawOption.y - difY
            );
            this._drawImage();
            this.touchStartFinger = {
                pageX01: pageX,
    ...
    touchEnd(e){
        ...
        this.isrect(this.drawOption.x, this.drawOption.y);
        this._drawImage();
    }
    //成功回调，返回裁剪画布base数据
    success(){
